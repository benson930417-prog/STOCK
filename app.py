...
def _latest_history_payload(ticker):
    if ticker in {"0050", "00830", "00878", "009805", "009820"}:
    ...
    with tab_master_holding:
        ...
        if portfolio_positions.empty:
            ...
            for _, pos in portfolio_positions.dropna(subset=["market_value"]).iterrows():
                ticker = pos.get("ticker")
                if ticker in {"00981A", "00988A", "00997A", "0050", "00830", "00878", "009805", "009820"}:
                    _, payload = _latest_history_payload(ticker)
                                        ...
                                        if st.button("刷新資料", key=f"btn_refresh_contribution_data_{title_prefix}", use_container_width=True):
                                            ...
                                            if "ticker" in portfolio_positions.columns:
                                                etf_tickers = [
                                                    ...
                                                    if str(ticker) in {"00981A", "00988A", "00997A", "0050", "00830", "00878", "009805", "009820"}
                                                ]
