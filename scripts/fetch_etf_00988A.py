...
HISTORY_FILE = os.path.join(DATA_DIR, "etf_00988A_history.json")
LOG_FILE = os.path.join(DATA_DIR, "etf_00988A_log.json")
ETF_TICKER = "00988A"
FUND_CODE = "61YTW"
...
def fetch_and_update_holdings():
    ...
    try:
        ...
        for i, row in df_raw.iterrows():
            ...
            if "代碼" in row_str or "名稱" in row_str or "浠ｇ⒓" in row_str or "鍚嶇ū" in row_str:
                header_row = i
                break
        ...
        for i in range(header_row):
            ...
            if "日期" in row_vals or "Date" in row_vals or "鏃ユ湡" in row_vals:
                import re
            ...
            elif "淨資產" in row_vals or "規模" in row_vals or "娣ㄨ硣鐢" in row_vals or "瑕忔ā" in row_vals:
                for val in row_list:
            ...
            elif "淨值" in row_vals or "NAV" in row_vals or "娣ㄥ€" in row_vals:
                for val in row_list:
                    if pd.notna(val) and ("NTD" in str(val) or any(c.isdigit() for c in str(val))):
                        try:
                            v_str = str(val).replace("NTD", "").replace(",", "").strip()
        ...
        for r in records:
            ...
            for k, v in r.items():
                k_str = str(k)
                if "代碼" in k_str or "代號" in k_str or "股票代碼" in k_str or "股票代號" in k_str or "浠" in k_str or "鑲＄エ浠ｇ⒓" in k_str:
                    sid = str(v).strip()
                elif "名稱" in k_str or "股票名稱" in k_str or "鍚" in k_str or "鑲＄エ鍚嶇ū" in k_str:
                    sname = str(v).strip()
                elif "權重" in k_str or "比例" in k_str or "娆婇噸" in k_str or "姣斾緥" in k_str or "%" in k_str:
                    try:
                        weight = float(str(v).replace("%", ""))
                    except Exception:
                        pass
                elif "股數" in k_str or "持股" in k_str or "鑲℃暩" in k_str or "鎸佽偂" in k_str:
                    try:
                        shares = int(float(str(v).replace(",", "")))
                    except Exception:
                        pass
        ...
        if is_changed:
            ...
            print(f"Successfully updated {ETF_TICKER} holdings for {file_date_str}. Total stocks: {len(clean_records)}")
        else:
