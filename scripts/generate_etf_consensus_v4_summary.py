#!/usr/bin/env python3
"""Render three complete mobile V4 lanes as cached LINE JPGs."""
...
OUT_PATHS = {
    "watching": SUMMARY_DIR / "etf_consensus_v4_watch_latest.jpg",
    "buying": SUMMARY_DIR / "etf_consensus_v4_buy_latest.jpg",
    "selling": SUMMARY_DIR / "etf_consensus_v4_sell_latest.jpg",
}
...
def render_lane_html(payload: dict, lane_key: str) -> str:
    ...
    lane_html = render_v4_lane(payload, lane_key)
...
<head>
  ...
  <style>
    * {{ box-sizing:border-box; }}
    ...
    {V4_CSS}
    .tfv4-lane {{ margin:0; background:rgba(15,23,42,.62); }}
    .tfv4-card {{ background:#101622; padding:16px 17px; margin:11px 0; }}
    .tfv4-title {{ font-size:24px; }}
    .tfv4-count {{ min-width:36px; height:36px; font-size:19px; }}
    .tfv4-note {{ font-size:15px; }}
    .tfv4-stock {{ font-size:23px; }}
    .tfv4-code,.tfv4-score {{ font-size:14px; }}
...
def generate(
    ...
    if set(paths) != set(V4_LANES):
        raise ValueError("V4 output paths must contain watching/buying/selling")
    ...
    with sync_playwright() as playwright:
        ...
        for lane_key in V4_LANES:
            ...
            if rendered != expected:
                browser.close()
                raise RuntimeError(
                    f"V4 {lane_key} card mismatch: expected {expected}, "
                    f"rendered {rendered}"
                )
