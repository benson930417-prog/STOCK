...
def fetch_and_update_00878():
    ...
    if not assets_data.get("success") or not assets_data.get("result"):
        raise ValueError(f"Failed to fetch 00878 assets from API: {assets_data.get('returnMessage')}")
    ...
    if not api_data.get("success") or not api_data.get("result"):
        raise ValueError(f"Failed to fetch 00878 holdings from API: {api_data.get('returnMessage')}")
