...
The rotation model deliberately has no decision interval. It gives every old session a gradually fading weight instead of dropping an entire buy wave at a boxcar boundary: 3-session half-life = current pressure, 10-session half-life = underlying direction, and 20-session half-life = background. "Strong/weak" magnitude is relative to that category's own prior smoothed-pressure history; a cross-category rank is secondary context. A coordinated buy/sell claim needs at least two selected ETFs pointing the same way, and a new phase must persist for two common sessions before it replaces the confirmed state. Separately, a latest-three-session sell-shock check surfaces abrupt reductions immediately—even while the slow background is still positive—without falsely calling that a confirmed long-term downtrend. The dashboard, daily email, generated image, and LINE reply must all call this shared engine. The 3/5/10/20 totals and the image's last-ten-session mini bars remain audit evidence only.
