...
| `src/ui/tag_flow_tab.py` | 題材流向 tab: a concise three-lane board (`綜合買盤已確認／綜合轉強待確認／3日 EWMA 目前減碼警示`) plus a category trend drill-down. The heading must say `最新…截至 YYYY-MM-DD`, never `今日`, because lane assignment is a 3/10/20-half-life combined state rather than a one-day result. Every board card shows the same 3-session EWMA current pressure and ETF buy/sell breadth; there is no literal recent-3-day total. The evidence flow is fixed: **② choose the audit interval → ③ show the interval category ranking openly (never hide it in an expander) → select a category from that ranked interval → show its trend and ④ interval-specific stock sources**. The selector changes only the ranking/chart/audit totals, never the category phase or LINE conclusion. The visible presets deliberately omit 3 and 5 sessions; only supported 1/10/20/60/120/240 ranges appear. Ranks normalized 相對力道 for fair cross-fund comparison and shows estimated 億元 only as intuitive context. Taiwan colors: red = 加碼/買進, green = 減碼/賣出, amber = pending. 概念 labels are stock-level notes only. Pure render of `data/tag_flow.json` (no network). |
