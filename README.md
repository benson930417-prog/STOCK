...
| `src/ui/tag_flow_v2_tab.py` | `ETF 動作` tab (the filename is historical): an insight-only three-lane stock lifecycle radar (`買進觀察／續抱參考／賣出警示`) with no interval selector or analytical panel beyond each stock's 20-session evidence sparkline. It shows every qualified card, including trigger continuation, one-buy-away hold watches, active holds, near-downgrade countdowns, just-downgraded holds, and hold-to-sell transitions. Its lane/card renderer is shared exactly with the three cached on-demand LINE images. |
...
18. **`ETF 動作` is a stateful action lifecycle, not a two-day event list, and it is the only ETF-flow insight sent to LINE.** `src/tag_flow_events.py` is the single evidence source; the website and `scripts/generate_etf_action_summary.py` share `src/tag_flow_action_cards.py`. The on-demand `ETF動作` reply is currently exactly **3 mobile-width vertical images**, ordered buy, hold, sell. The cached text remains available but `ETF_ACTION_INCLUDE_TEXT = False` temporarily keeps it out of the on-demand reply. Each image renders one complete website lane with every qualified card; computed-height capture and rendered-card-count verification are mandatory, and fixed crops/`max-height`/`overflow:hidden` are forbidden. The webhook serves cached images only and must never recalculate or screenshot during a reply. The scheduled daily broadcast remains one combined report/action text plus exactly four active-ETF images.
    ...
    **Hold is a rolling lifecycle.** Entry/continuation requires at least 2 participating ETFs, at least 4 significant buy days in the latest 10 common sessions, buy days at least 2 greater than sell days, and normalized net flow of at least `+0.20%`. The latest session no longer has to be a buy: a hold survives a quiet day while its 10-session evidence still qualifies. Every relevant positive card shows the missing buy days/ETF breadth/net strength; a one-buy-away card appears only when one additional significant buy day would satisfy the count/balance and all other criteria already pass. Active holds show how many quiet sessions remain before rolling evidence would fail. A just-failed hold remains visible for one session as `今日降級`; if a qualified sell simultaneously forms, it moves to sell as `續抱→賣出警示` rather than disappearing. `data/etf_action_history.json` upserts the latest two daily snapshots and retains 260 sessions for audit.

    Every card follows `類股／動作／ETF／判定／狀態／門檻／進度（when relevant）／依據`. Its 20-session micro chart sums the three ETFs' normalized flows (red buy, green sell), uses a local per-stock vertical scale, frames the original strategy evidence, and adds a second neutral frame for current continuation. The 10-session hold bracket is evidence, not a selectable chart interval. Category is context only; concepts are never interpreted. Never infer high/low purchase price from ETF holdings alone.
