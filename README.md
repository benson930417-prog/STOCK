...
| `ETF動作` / `ETF 動作` | Exactly four reply objects in this order: the complete cached action text, then three mobile-width vertical images for `買進觀察`, `續抱參考`, and `賣出警示`. Each image is only its website lane—no repeated methodology/header block—and includes every card in that lane. 類股 is context only and 概念 is never interpreted. The three JPGs are regenerated daily and served from cache; the webhook never screenshots or recalculates on demand. |
...
| `data/summaries/etf_action_{buy,hold,sell}_latest.jpg` | Three latest mobile-width `ETF 動作` lane images. Together they contain every event in the qualified board; they are regenerated and committed daily, then served after the restored text in the free on-demand LINE reply. |
