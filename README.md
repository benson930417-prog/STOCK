...
| `scripts/generate_etf_action_insight.py` | Converts `data/tag_flow.json` through the stock-level action engine into cached `data/etf_action_insight.json`. Its `line_text` is the concise buy/hold/sell insight appended inside the daily LINE report's first text object and served by the on-demand `ETF動作` reply. It never creates an image and never sends LINE itself. |
