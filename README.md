...
| `src/etf_consensus_v4.py` | Final state/history engine. It reuses V3's flow-adjusted, no-look-ahead per-ETF gates, then cleanly separates three hard states: yellow is one high-information manager precursor, red is independently confirmed buying, and green is independently confirmed selling. Red/green require at least 2/3 individually significant signals whose three-common-session lifetimes overlap; a yellow score can never bypass that gate. Fixed 3/10/20 EWMA half-lives describe current pressure/direction/background, while an auditable 0–100 maturity score ranks signals inside the same colour. The engine keeps 260-session date-keyed boards, compact state transitions, and per-ETF normalized history for replay. |
| `src/etf_consensus_v4_cards.py` | Shared final V4 card renderer. Red/green cards show three aligned ETF microcharts; yellow shows only its triggering ETF. Bar height is each ETF/stock's own threshold multiple, not absolute dollars. A pale band marks the three-session overlap window and a bright frame marks the latest actual significant bar—there are no hard-coded empty frames. |
...
| `src/ui/etf_consensus_v4_tab.py` | `ETF 共識 V4` final tab. The only selector is a historical trading-date replay; it cannot change the fixed 3/10/20 model or rewrite the story. The frontend explicitly explains the hard colour gate, non-probability score, maintenance/downgrade rules, and own-threshold chart scale. |
...
- **ETF 共識 V4** is the final copy-the-manager surface. It keeps a historical state rather than a daily lottery: yellow is a high-information one-manager watch, while red/green require at least two independent managers. Ordinary holdings and ordinary 1/3 activity remain hidden.
...
| `ETF共識` / `ETF意圖` | Exactly three image reply objects and no text: yellow `單一 ETF 觀察`, red `買方共識`, then green `賣方共識`. Images contain every current V4 state card and are rebuilt daily from `data/etf_consensus_v4.json`. The webhook only serves cache; it never calculates or screenshots on demand. The rich-menu action uses `ETF共識`; legacy `ETF意圖` aliases intentionally route to the same final result. |
...
| `scripts/generate_etf_consensus_v4_summary.py` | Renders the shared V4 website cards into three uncropped mobile images (watch/buy/sell), verifies every cached card was rendered, and saves them for on-demand LINE reply. |
...
| `data/etf_consensus_v4.json` | Final V4 state/history cache: date-keyed yellow/red/green boards, hard-gate methodology, transparent score components, compact state-transition audit, and up to 260 aligned per-ETF own-threshold sessions. Rebuilt daily by `scripts/build_etf_consensus_v4.py`. |
...
| `data/summaries/etf_consensus_v4_{watch,buy,sell}_latest.jpg` | Three complete V4 lanes used by the free on-demand `ETF共識` / `ETF意圖` LINE reply. No text object is included. |
...
21. **`ETF 共識 V4` is the final copy-the-manager contract.** Colour and score have different jobs. Yellow/watch is allowed only for one-manager structural/re-entry/full-exit/reversal/restart evidence or a former consensus that lost its second manager. To avoid tiny holding-list cleanup washing the lane, new/re-entry needs at least `0.5×` its own normal-action threshold and full exit needs `1.0×`; reversal/restart already requires the complete significance gate. Red/green require at least **2/3 ETFs**, each already significant versus its own no-look-ahead baseline, with same-direction signal lifetimes overlapping within three common sessions. A score—however large—must never upgrade 1/3 to red/green. Fixed EWMA half-lives are 3 (current pressure), 10 (underlying direction), and 20 (background); never add an interval selector that changes the conclusion. The only selector is date-keyed historical replay. Confirmed state is maintained only while at least two managers still support the 10-session direction and consensus score remains at least 40; one remaining manager downgrades to yellow, zero removes the card. Red/green cards always show three aligned ETF rows; yellow shows only its triggering manager. Chart height is clipped own-threshold ratio, not absolute 億元 or a cross-ETF raw sum. The LINE command/rich-menu reply is exactly **three cached images** in watch/buy/sell order, no text, no fetch, no screenshot, and no paid push/broadcast.
