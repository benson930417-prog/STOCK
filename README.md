...
| `src/etf_consensus_v4.py` | Final state/history engine. It reuses V3's flow-adjusted stock moves, then assigns V4's own no-look-ahead ETF-wide usual-action scale from the preceding 10 common sessions. Yellow is one high-information manager precursor, red is independently confirmed buying, and green is independently confirmed selling. Red/green require at least 2/3 individually significant signals whose three-common-session lifetimes overlap; a yellow score can never bypass that gate. Maintenance requires the participating ETF/stock 10-session net derivative to remain in the state direction, while freshness and the displayed expiry fall on every quiet session. Fixed 3/10/20 EWMA half-lives describe current pressure/direction/background. The engine keeps 260-session date-keyed boards, compact state transitions, and per-ETF normalized history for replay. |
| `src/etf_consensus_v4_cards.py` | Shared final V4 card renderer. Red/green cards show three aligned ETF microcharts; yellow shows only its triggering ETF. Bar height is the move divided by that ETF's preceding-10-session **usual non-zero single action**, never the significance-gate multiple or absolute dollars. Evidence also shows estimated 億元 for intuition, the ETF's usual single-action 億元, its 10-session net derivative, and a no-new-action expiry countdown. A pale band marks the three-session overlap window and a bright frame marks the latest actual significant bar—there are no hard-coded empty frames. |
...
| `src/ui/etf_consensus_v4_tab.py` | `ETF 共識 V4` final tab. The only selector is a historical trading-date replay; it cannot change the fixed 3/10/20 model or rewrite the story. The frontend explicitly explains the hard colour gate, non-probability score, 10-session net-derivative maintenance/downgrade rules, daily freshness decay, and usual-single-action chart scale. |
...
| `data/etf_consensus_v4.json` | Final V4 state/history cache: date-keyed yellow/red/green boards, hard-gate methodology, transparent score components (including freshness), compact state-transition audit, and up to 260 aligned per-ETF usual-action sessions. Rebuilt daily by `scripts/build_etf_consensus_v4.py`. |
