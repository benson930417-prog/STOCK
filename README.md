...
| `src/ui/tag_flow_v2_tab.py` | `ETF 動作` tab (the filename is historical): an insight-only three-lane stock radar (`買進觀察／續抱參考／賣出警示`) with no interval selector and no charts. It shows the engine's complete qualified board (up to six fresh events per buy/sell side and four exceptional continuing-buy signals in the hold lane), their single 類股 as context, the ETFs involved, and plain-language evidence. Its card renderer is shared with the cached on-demand LINE image. |
...
| `ETF動作` / `ETF 動作` | Latest cached stock-level buy/hold/sell board as one full-height image reply. It uses the exact card renderer and complete qualified stock list from the dashboard's `ETF 動作` tab, includes 類股 only as context, and never uses 概念 labels. Available as a visible Page 1 rich-menu tile and an 吳大師 quick reply. The JPG is regenerated in the daily job and served from cache; the webhook never screenshots or recalculates on demand. |
...
| `scripts/generate_etf_action_summary.py` | Renders `data/etf_action_insight.json` through the same shared three-lane card renderer as the website and writes `data/summaries/etf_action_latest.jpg`. It captures the computed body height, verifies rendered-card count against the cache, and therefore must never crop the last card. This cached JPG is the on-demand `ETF動作` reply. |
...
| `data/summaries/etf_action_latest.jpg` | Latest full-height `ETF 動作` board. It contains every event in the cached qualified board, is regenerated and committed by the daily job, and is served as the free on-demand LINE reply. |
...
8. Run a live Cmoney category canary, validate category coverage, build the shared flow observations, generate the complete stock-level `ETF 動作` cache, then render its full-height LINE image.
...
18. **`ETF 動作` is the actionable buy/hold/sell evidence radar and the only ETF-flow insight sent to LINE.** `src/tag_flow_events.py` is the single evidence source; the website and `scripts/generate_etf_action_summary.py` share `src/tag_flow_action_cards.py`, so the on-demand LINE `ETF動作` command returns the same complete three-lane card board as one cached full-height JPG. The image renderer must verify its `.tfv2-card` count against every event in `data/etf_action_insight.json`, capture the body's computed height, and never use a fixed-height crop, `max-height`, or `overflow:hidden`; the last card must always be present. The webhook only serves `data/summaries/etf_action_latest.jpg` and must never render or recalculate during a reply. The scheduled daily broadcast remains different because it already uses LINE's five-object limit: it inserts the cached action text beneath the compact report list inside the first text object, followed by exactly four active-ETF images. That text and the admin email retain the fixed field order `名稱｜代號`, `類股`, `動作`, `ETF`, `判定`, `依據`; do not restore event-specific labels or automatic wrapping. The three decisions are: `買進觀察` for a fresh disclosed entry/re-entry/reversal/restart, `續抱參考` for exceptional continuing conviction, and `賣出警示` for a fresh exit/reversal/restart. A `new_position` in one daily comparison means newly present, not necessarily first-ever; if that ETF has any earlier `full_exit` in its full available history, label the event `重新建倉`. Cross-ETF directional comparisons remain restricted to common sessions. Fresh buy/sell events live for exactly the latest **2 common ETF sessions**: the UI labels them `本交易日確認` then `前一交易日確認`, and they disappear on the next common session; there must never be a `3 日前確認` state. The hold lane is different: `最新資料仍確認` is recalculated from scratch every day and is not an old event retained by age. Ordinary adjustments must clear `max(0.02% ActiveWeight, 35% of that ETF's trailing trade median)`; tiny full-exit residue is hidden. **Qualification is fixed and user-visible:** an ordinary fresh action requires at least 2/3 ETFs aligned. A 1/3 action is hidden unless it is a holding-list change (`新建倉／重新建倉／完整出清`) or a genuine reversal (`賣後轉買／買後轉賣`) that persists for two common sessions. A single-ETF ordinary restart must not surface. Ordinary continuations stay out of fresh buy/sell lanes; the hold lane requires participation by at least 2 ETFs, at least 4 meaningful buy days in the latest 10 common sessions, buy days at least 2 greater than sell days, normalized net flow of at least 0.20, and a latest-session buy confirmation. Both the web page and LINE image must state the 2/3 rule and the narrow 1/3 exceptions. The page deliberately has no date interval, chart, table, or cash-size ranking. 類股 is context only; 概念 is never interpreted. Never infer that a trade occurred at a price high/low from ETF holdings alone; price timing requires separately verified market prices.
