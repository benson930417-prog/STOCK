...
| `src/tag_flow_events.py` | Stock-level V2 action-event engine. It emits only fresh new positions, meaningful full exits, confirmed reversals against the prior 20-session behaviour, and restarts after at least 5 inactive sessions. Ordinary small adjustments and continuing buys/sells are excluded. A new position is a hard structural event; other direction changes require 2 ETFs on the same disclosure or 2 consecutive sessions. Category is context only and concepts are never interpreted. |
...
| `src/ui/tag_flow_v2_tab.py` | Separate `題材流向 V2` tab; V1 remains intact. This is an insight-only two-lane stock radar (`剛開始買／剛開始賣`) with no interval selector and no charts. It shows at most six fresh confirmed events per side, their single 類股 as a context pill, the ETFs involved, and the plain-language confirmation reason. |
...
18. **題材流向 V2 is a fresh stock-action radar, not another level/ranking dashboard.** Keep V1 and V2 separate. `src/tag_flow_events.py` is the only source of V2 events, and `src/ui/tag_flow_v2_tab.py` only renders them. V2 may show: a newly disclosed position (including a clearly labelled tiny `試單建倉`), a meaningful full exit, a confirmed reversal versus the prior 20 common sessions, or a confirmed restart after at least 5 inactive sessions. Ordinary adjustments must clear `max(0.02% ActiveWeight, 35% of that ETF's trailing trade median)`; tiny full-exit residue is hidden. Non-structural direction changes require either 2 ETFs on the same disclosure or the same direction for 2 consecutive sessions. Once a direction is already confirmed, continuing buys/sells must disappear rather than remain ranked. The page deliberately has no date interval, chart, table, or cash-size ranking. It shows at most six events per side from the latest two common sessions. 類股 is a context label only; 概念 is never interpreted.
