...
| `src/etf_intent_v3.py` | Profit-research-oriented V3 stock intent engine. It first removes mechanical portfolio scaling from ETF creations/redemptions using exact disclosed outstanding units (`fund_size / NAV` only for historical rows that predate the field), keeps only actual share trades whose direction agrees with the flow-adjusted allocation residual plus structural entry/exit, applies a no-look-ahead 20-common-session stock/ETF gate, and maintains hidden buy/neutral/sell regimes. It emits only new onset, re-entry, reversal, restart, acceleration, ETF-breadth expansion, and at most one next-session follow-through. It never emits a visible hold basket and never reads concepts. |
...
| `src/ui/etf_intent_v3_tab.py` | `ETF 意圖 V3` tab: two visible lanes only (`新買方意圖／新賣方意圖`). Existing positions, ordinary continuing buys, and cooling without an actual sell stay hidden. Each card states the transition, ETF drivers, evidence score (explicitly not a return probability), estimated TWD context, unit-data quality, and a 20-session flow-adjusted residual sparkline. |
...
- **ETF 意圖 V3** is the copy-the-manager transition surface. It has only two visible lanes and never lists a stock merely because an ETF owns it. A hidden regime is still required so “stopped buying” is not mislabeled as “started selling.”
...
All three active-ETF source workbooks disclose exact outstanding units. The fetchers now persist `meta.outstanding_units`; historical records created before this field use the auditable `fund_size / rounded NAV` estimate and carry `unit_quality=estimated`. V3 stores both raw and adjusted directions. A residual that opposes the actual share trade remains audit context but is not shown as a copyable buy/sell. The only structural exceptions are an actual holding-list entry or full exit.

Each ETF/stock move is judged with no look-ahead against its preceding 20 common sessions. Evidence is aggregated continuously: two ordinary significant ETFs can qualify, one extremely abnormal ETF can qualify, and a single structural entry/exit or genuine reversal can qualify. Conflicting directions are suppressed unless one side clearly dominates. The backend keeps decaying buy/neutral/sell state to distinguish onset, reversal, restart, acceleration, and breadth expansion. A next-session same-direction action may appear once as follow-through; uninterrupted mature accumulation does not remain visible. The V3 score measures evidence strength only and must never be called a calibrated return probability.
...
| `ETF意圖` / `ETF 意圖` | Exactly two reply objects: the complete mobile-width `新買方意圖` image followed by the complete `新賣方意圖` image. There is no text object and no hold image. The current rich-menu action points here; the old `ETF動作` command remains available for lifecycle audit. Images are rebuilt daily from `data/etf_intent_v3.json`; the webhook only serves cache. |
...
| `data/etf_intent_v3.json` | V3 flow-adjusted intent cache (schema 3): exact/estimated ETF-unit quality, two fresh signal lanes, the complete historical event log, and hidden-regime audit rows. Rebuilt daily by `scripts/build_etf_intent_v3.py`. |
...
| `data/summaries/etf_intent_v3_{buy,sell}_latest.jpg` | Two complete V3 intent lanes used by the free on-demand `ETF意圖` LINE reply. No text and no hold image are included. |
...
The rich menu uses three LINE rich-menu aliases. Page 1 is fast market/macro plus direct `市場脈動`, `吳大師`, and `ETF 意圖` actions; Page 2 is the primary ETF watchlist, and Page 3 is ETF overflow. Navigation convention is fixed: previous page is bottom-left, next page is bottom-right, and the last page uses bottom-right `首頁` until another page is needed. Page 1's narrow five-column row deliberately uses short labels (`新買・新賣`, `那斯達克` + `24 小時`); both title and subtitle fonts are width-fitted per tile. The former three-lane `ETF動作` remains available as a typed command for audit.
...
19. **`ETF 意圖 V3` is a two-lane transition detector, not another holdings or hold list.** Exact outstanding ETF units must be stored by all three active fetchers and used before taking any stock-flow derivative. Historical `fund_size / NAV` estimates must retain their `estimated` label. V3 may show only copyable actual-share actions plus entry/exit; relative dilution/defence that opposes the real share direction stays audit-only. It has hidden buy/neutral/sell state but visible `buying` and `selling` lanes only. Continuing accumulation appears at trigger and at most one next-session follow-through, then stays hidden until restart, acceleration, breadth expansion, or reversal. The rich-menu `ETF 意圖` action returns exactly **2 cached images**, buy then sell, with no text and no hold card. Keep the old typed `ETF動作` command available. Neither webhook command may fetch, calculate, screenshot, or send paid push/broadcast messages.
