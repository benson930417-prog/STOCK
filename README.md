...
| `src/ui/tag_flow_tab.py` | 題材流向 tab: a concise three-lane board (`綜合買盤已確認／綜合轉強待確認／近3日減碼警示`) plus a category trend drill-down. The heading must say `最新…截至 YYYY-MM-DD`, never `今日`, because lane assignment is a 3/10/20-half-life combined state rather than a one-day result. Every board card shows the same literal latest-three-common-session `%規模` total and ETF buy/sell breadth; the lane label separately says whether it is a combined conclusion or the independent early sell warning. The evidence flow is fixed: **② choose the audit interval → ③ show the interval category ranking openly (never hide it in an expander) → select a category from that ranked interval → show its trend and ④ interval-specific stock sources**. The selector changes only the ranking/chart/audit totals, never the category phase or LINE conclusion. Only ranges supported by the current common history are shown; 60/120/240 appear automatically when enough sessions exist. Ranks normalized 相對力道 for fair cross-fund comparison and shows estimated 億元 only as intuitive context. Taiwan colors: red = 加碼/買進, green = 減碼/賣出, amber = pending. 概念 labels are stock-level notes only. Pure render of `data/tag_flow.json` (no network). |

題材流向 periods mean the latest **common trading sessions** across the selected ETFs, not calendar days; 20 trading sessions naturally span about one calendar month. If 60/120/240 exceeds the current shared history, those buttons stay hidden rather than silently showing the same one-month data. They appear automatically after enough common sessions accumulate. The selector is immediately above the category chart and is a chart/audit viewport only. The rotation story always uses all available common history through `src/tag_flow_rotation.py`, so switching 5/10/20 days cannot rewrite the headline.

The rotation model deliberately has no decision interval. It gives every old session a gradually fading weight instead of dropping an entire buy wave at a boxcar boundary: 3-session half-life = current pressure, 10-session half-life = underlying direction, and 20-session half-life = background. These are decay speeds, not literal truncated 3/10/20-day windows. "Strong/weak" magnitude is relative to that category's own prior smoothed-pressure history; a cross-category rank is secondary context. A coordinated buy/sell claim needs at least two selected ETFs pointing the same way, and a new phase must persist for two common sessions before it replaces the confirmed state. Separately, a literal latest-three-common-session sell-shock check surfaces abrupt reductions immediately—even while the slow background is still positive—without falsely calling that a confirmed long-term downtrend. For UI consistency, every board card prints this same literal three-session total and breadth, while the column heading distinguishes the combined model conclusion from the early sell alert. The dashboard, daily email, generated image, and LINE reply must all call this shared engine. The 3/5/10/20 totals and the image's last-ten-session mini bars remain audit evidence only.
...
| `data/tag_flow_insight.json` | Shared rotation-engine output (schema v2) used by email and LINE: confirmed category phases, confidence/breadth, own-history strength, 3/5/10/20 audit totals, display-only last-ten-session bars, and the rendered text payload. Rebuilt daily by `scripts/generate_tag_flow_insight.py`. |
...
17. **Dashboard, email, and LINE 題材洞察 share one window-independent rotation engine.** `src/tag_flow_rotation.py` is the only source of category phase, ranking, breadth, confidence, recent-sell warnings, and common-stock pools. `scripts/generate_tag_flow_insight.py` writes both `data/tag_flow_insight.json` and the tracked `data/summaries/tag_flow_insight_latest.jpg`; the daily email prints `email_text`, while the scheduled 18:30 LINE run and on-demand `題材洞察` reply serve the image. Never copy decision logic into the UI or webhook. The engine uses all common history with EWMA half-lives 3/10/20 for current pressure/underlying direction/background, judges magnitude against that category's own prior pressure history, requires majority-ETF direction breadth, and confirms a changed phase only after two consecutive common sessions. In addition, it computes a separate latest-3-session sell-shock alert so a large new reduction is visible immediately even while its background remains positive. Regression example as of 2026-07-21: 被動元件 was `-0.56% 規模 / 2 of 3 ETF selling` and must appear as a warning. This alert is explicitly short-term and does not bypass phase confirmation. User-facing cards use `自身力道強／中／一般`, never unexplained P80/P64 labels. The graph selector belongs directly above the trend chart; unsupported 60/120/240 ranges remain hidden until enough common history exists. Changing the graph range must not change phase, rank, or LINE wording. A stock enters `三檔共買池` or `三檔共賣池` only when 00403A, 00981A, and 00991A all agree under the same smoothed current-pressure rule. Concepts remain stock-level display notes only. Never run the full daily script merely to test this card because that would send the paid broadcast; render the generator directly and inspect the JPG instead.
