...
**Backtest surface:** The adjacent `ETF 共識回測` tab combines V4 state history with the tracked, one-time `data/yuanta_v4_daily_k.json` PROD snapshot. It is long-only and strictly no-look-ahead: a new buy state enters at the next available session open, and losing the buy state exits at the next available session open. It models finite equal-slot capital, commission, 0.3% sell tax, slippage, open-position mark-to-market, and a 0050 comparison. Historical Yuanta K lines contain OHLCV but no historical average price, so the engine must never label an invented VWAP as executable data. The latest three V4 sessions covered by the snapshot are audited for all 102-stock price coverage and valid OHLC geometry. Date controls stop at the snapshot boundary and visibly report when daily V4 signals have advanced beyond it. This snapshot is intentionally not refreshed by the Linux daily job because the official Yuanta component runs on the authorized local Windows machine.
...
| `src/etf_consensus_backtest.py` | Pure no-look-ahead V4 portfolio simulator and three-session Yuanta price audit. It schedules entries/exits only after signal disclosure, executes exits before ranked new entries, enforces finite equal-slot cash, and keeps open positions marked to the last close. |
...
| `src/ui/etf_consensus_backtest_tab.py` | `ETF 共識回測` tab next to V4. Provides capital, position-count, fee, slippage, and signal-range controls; strategy-vs-0050 curve; trade ledger; and the strict latest-three-session coverage/OHLC audit. It performs no network calls. |
...
| `data/yuanta_v4_daily_k.json` | One-time read-only Yuanta SPARK PROD daily-OHLCV snapshot for the 102-stock V4 universe plus 0050 (2026-03-01 through 2026-08-05 request; 103/103 symbols returned). It contains no account, password, certificate, order, or subscription data. Generated locally from the external `!YuantaAPIStuff` folder and tracked so the Linux dashboard can reproduce the backtest without holding brokerage credentials. |
