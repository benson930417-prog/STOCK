...
| `ETF意圖` / `ETF 意圖` | Exactly two reply objects: the complete mobile-width `買方共識` image followed by the complete `賣方共識` image. Both require at least 2/3 ETFs to act significantly in the same direction on the same common session, and each image separates today's new consensus from today's continued confirmation. There is no text object, no hold image, and no 1/3 exception. The current rich-menu action points here; the old `ETF動作` command remains available for lifecycle audit. Images are rebuilt daily from `data/etf_intent_v3.json`; the webhook only serves cache. |
...
19. **`ETF 意圖 V3` is a strict two-manager consensus detector, not another holdings, hold, or single-manager alert list.** Exact outstanding ETF units must be stored by all three active fetchers and used before taking any stock-flow derivative. Historical `fund_size / NAV` estimates must retain their `estimated` label. Actual share direction must agree with the adjusted residual, then at least **2/3 ETFs must independently pass their own no-look-ahead gates in the same direction on the same common session**. A 1/3 action is never visible—no extreme-size, new-position, re-entry, reversal, or full-exit exception. V3 has hidden buy/neutral/sell state but visible `buying` and `selling` lanes only. Each lane separates consensus newly formed on the latest session from prior-session consensus confirmed again today. Prior signals without current 2/3 confirmation disappear. The rich-menu `ETF 意圖` action returns exactly **2 cached images**, buy then sell, with no text and no hold card. Keep the old typed `ETF動作` command available. Neither webhook command may fetch, calculate, screenshot, or send paid push/broadcast messages.
