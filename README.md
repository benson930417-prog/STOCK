...
| `src/ui/tag_flow_tab.py` | 題材流向 tab: a window-independent 類股 rotation board plus flexible 1/5/10/20/60/120/240/all/custom chart and audit ranges. The range selector changes only visible charts and interval totals; it never changes the category phase, ranking, or LINE conclusion. Ranks normalized 相對力道 for fair cross-fund comparison and shows estimated 億元 only as intuitive context. Uses Taiwan colors consistently: red = 加碼/買進, green = 減碼/賣出, amber = transition. 概念 labels appear only as stock-level notes and never enter interpretation. Pure render of `data/tag_flow.json` (no network). |

題材流向 periods mean the latest **common trading sessions** across the selected ETFs, not calendar days. When 60/120/240 days exceeds the shared history currently available, the UI uses every available common session and states that limitation; it must never pad, duplicate, or silently imply that the longer history exists. The selector is a chart/audit viewport only. The rotation story always uses all available common history through `src/tag_flow_rotation.py`, so switching 5/10/20 days cannot rewrite the headline.

The rotation model deliberately has no decision interval. It gives every old session a gradually fading weight instead of dropping an entire buy wave at a boxcar boundary: 3-session half-life = current pressure, 10-session half-life = underlying direction, and 20-session half-life = background. "Strong/weak" magnitude is relative to that category's own prior smoothed-pressure history; a cross-category rank is secondary context. A coordinated buy/sell claim needs at least two selected ETFs pointing the same way, and a new phase must persist for two common sessions before it replaces the confirmed state. The dashboard, daily email, generated image, and LINE reply must all call this shared engine. The 3/5/10/20 totals and the image's last-ten-session mini bars remain audit evidence only.
...
| `題材洞察` | Latest decision-only image card from the shared window-independent rotation engine: confirmed buy stages, genuine confirmed selling stages when present, transitions, and strict 3/3 common buy/sell stock pools. The ten-session mini bars are visual history only and never decide the story. Available as a visible Page 1 rich-menu tile and as an 吳大師 quick reply. |
...
17. **Dashboard, email, and LINE 題材洞察 share one window-independent rotation engine.** `src/tag_flow_rotation.py` is the only source of category phase, ranking, breadth, confidence, and common-stock pools. `scripts/generate_tag_flow_insight.py` writes both `data/tag_flow_insight.json` and the tracked `data/summaries/tag_flow_insight_latest.jpg`; the daily email prints `email_text`, while the scheduled 18:30 LINE run and on-demand `題材洞察` reply serve the image. Never copy decision logic into the UI or webhook. The engine uses all common history with EWMA half-lives 3/10/20 for current pressure/underlying direction/background, judges magnitude against that category's own prior pressure history, requires majority-ETF direction breadth, and confirms a changed phase only after two consecutive common sessions. The 1/5/10/20/60/120/240/all/custom selector, 3/5/10/20 totals, and ten-session image bars are display/audit tools only; changing them must not change phase, rank, or LINE wording. A positive background with negative current pressure is a transition/recent-selling state, not a window contradiction. Slowing positive flow is never called confirmed selling until current pressure, ETF breadth, and the two-session rule agree. A stock enters `三檔共買池` or `三檔共賣池` only when 00403A, 00981A, and 00991A all agree under the same smoothed current-pressure rule. Concepts remain stock-level display notes only. Never run the full daily script merely to test this card because that would send the paid broadcast; render the generator directly and inspect the JPG instead.
