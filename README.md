...
**Backtest surface:** The adjacent `ETF 策略回測` tab is a shared, long-only, strictly no-look-ahead portfolio engine backed by the tracked, one-time `data/yuanta_v4_daily_k.json` PROD snapshot. The strategy selector contains (1) V4 three-manager buy consensus and (2) the deliberately separate `00981A 續買特殊版`. The 981 variant replays all available 00981A disclosure pairs: actual shares must increase and the creation/redemption-adjusted active-allocation residual must also be positive; any such increase is actionable without a significance or consensus gate, and the next disclosure without another qualifying increase ends the state. Both variants enter and exit at the next available session open. The engine models finite equal-slot capital, commission, 0.3% sell tax, slippage, open-position mark-to-market, and a 0050 comparison. Historical Yuanta K lines contain OHLCV but no historical average price, so the engine must never label an invented VWAP as executable data. Each strategy audits its latest three snapshot-covered signal sessions against its complete stock universe. Date controls stop at the snapshot boundary and visibly report when daily signals have advanced beyond it. This snapshot is intentionally not refreshed by the Linux daily job because the official Yuanta component runs on the authorized local Windows machine.
...
| `src/etf_981_follow_strategy.py` | Special 00981A-only signal adapter. Replays the full 981 disclosure history with the shared flow-adjusted primitive and marks a stock `buy` only when actual shares and creation/redemption-adjusted active allocation both rise; the first non-buy disclosure ends the state. It intentionally has no significance or cross-ETF consensus gate. |
| `src/etf_consensus_backtest.py` | Strategy-agnostic no-look-ahead portfolio simulator and three-session Yuanta price audit. It schedules entries/exits only after signal disclosure, executes exits before ranked new entries, enforces finite equal-slot cash, and keeps open positions marked to the last close. |
...
| `src/ui/etf_consensus_backtest_tab.py` | `ETF 策略回測` tab next to V4. Switches between V4 consensus and the 00981A-only special strategy while sharing capital, position-count, fee, slippage, signal-range, strategy-vs-0050 curve, trade ledger, and latest-three-session coverage/OHLC audit controls. It performs no network calls. |
