...
- NASDAQ must set the `1200x900` viewport before navigation, click TradingView's accessible `1 day` button, and verify its selected state before hiding page chrome. A `?timeframe=1D` URL alone is insufficient. Failed verification preserves the last valid cache.
...
| Webhook cache freshness | `get_cached_market_chart(..., max_age_seconds=240)` | open markets: serve up to **240s** old; NASDAQ's checksum-verified Friday chart remains valid only during IG's real weekend closure |
...
**Shared-upstream backoff.** A TradingView 403 is returned by `chart_service.py` as an explicit 503. The monitor stops that shared-upstream cycle at the first blocked key and uses persistent exponential cooldown (5, 10, 20, then at most 30 minutes). The cooldown survives service restarts and resets only after a non-blocked shared cycle, so a deploy cannot accidentally restart a request storm. NASDAQ uses an independent IG page and continues refreshing every 60 seconds during the TradingView cooldown; its success cannot clear the TradingView outage state. Existing checksum-verified caches stay untouched during an outage.

**NASDAQ timeframe is verified, not assumed.** TradingView may open `?timeframe=1D` while the actual range button still has `1 year` selected. The NASDAQ snapshot path sets its desktop viewport before navigation, waits for the accessible `1 day` button, clicks it while still visible, and verifies its `selected-*`/ARIA state before hiding page chrome. Each retry navigates from the canonical IG URL and rechecks upstream-block content. Missing/failed selection rejects that refresh and preserves the previous valid cache; a merely nonblank long-range chart is never sufficient.

**Snapshot crop rules.** Generic 1-month charts (`oil`, `brent`, `bond`, `gold`, `usdtwd`, `usdjpy`, `usdchf`) may detect the chart's `y` and `height`, but they must use the full TradingView viewport width: `clip.x = 0` and `clip.width = window.innerWidth`. Their LINE quote text remains the same four performance rows (`1日`, `1週`, `1月`, `6月`); only the plotted chart window is one month. Do not crop or shift the x-axis for generic charts; the right price axis and last-price marker live at the far right edge. NASDAQ is the exception: it keeps the separate IG 24h branch and fixed `1200` width required by `overlay_market_sessions.py`, but measures `y`/`height` from the actual selected chart canvas. A fixed NASDAQ `y` is forbidden because TradingView page chrome moves the canvas.
