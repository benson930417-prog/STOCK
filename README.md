...
The rich menu uses three LINE rich-menu aliases. Page 1 is fast market/macro plus direct `市場脈動`, `吳大師`, and `ETF 動作` actions; Page 2 is the primary ETF watchlist, and Page 3 is ETF overflow. Navigation convention is fixed: previous page is bottom-left, next page is bottom-right, and the last page uses bottom-right `首頁` until another page is needed.
