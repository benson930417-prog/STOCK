...
| `src/ui/tag_flow_v2_tab.py` | `ETF 動作` tab (the filename is historical): an insight-only three-lane stock radar (`買進觀察／續抱參考／賣出警示`) with no interval selector and no charts. It shows at most six fresh events per buy/sell side and four exceptional continuing-buy signals in the hold lane, their single 類股 as context, the ETFs involved, and plain-language evidence. This is the actionable surface and the source of the daily cached LINE text. |
...
| `ETF動作` / `ETF 動作` | Latest cached stock-level buy/hold/sell insight as one text reply. It uses the same evidence engine as the dashboard's `ETF 動作` tab, includes 類股 only as context, and never uses 概念 labels. Available as a visible Page 1 rich-menu tile and an 吳大師 quick reply. No category-rotation image is generated or served. |
...
| `scripts/generate_etf_action_insight.py` | Converts `data/tag_flow.json` through the stock-level action engine into cached `data/etf_action_insight.json` (schema v2). Its `line_text` is the concise buy/hold/sell insight appended inside the daily LINE report's first text object and served by the on-demand `ETF動作` reply. Every stock uses the fixed `類股／動作／ETF／判定／依據` field order. It never creates an image and never sends LINE itself. |
...
| `data/etf_action_insight.json` | Cached stock-level buy/hold/sell result and ready-to-send LINE text, rebuilt daily by `scripts/generate_etf_action_insight.py`. This is the only tag-flow cache consumed by LINE; category rotation has no LINE cache or image. |
...
8. Run a live Cmoney category canary, validate category coverage, build the shared flow observations, and generate the cached stock-level `ETF 動作` text.
...
18. **`ETF 動作` is the actionable buy/hold/sell evidence radar and the only ETF-flow insight sent to LINE.** `src/tag_flow_events.py` is the single evidence source; `src/ui/tag_flow_v2_tab.py` renders it and `scripts/generate_etf_action_insight.py` writes its concise schema-v2 cached LINE text to `data/etf_action_insight.json`. The on-demand `ETF動作` command returns text only. The scheduled daily broadcast inserts that cached text beneath a compact four-line report list inside the first text object, followed by exactly four active-ETF images. LINE text deliberately mirrors the proven `吳大師` hierarchy: plain title, heavy divider, dated header, named sections with counts, and zero-padded numbered stocks. **Never rely on LINE's automatic wrapping.** Every stock uses exactly the same rows and order: `名稱｜代號`, `類股`, `動作`, `ETF` (who acted), `判定` (why it was admitted), and `依據` (short supporting facts). Do not replace those fields with event-specific labels such as `同步／10日／參與／狀態`. A complete evidence fact must remain on one deliberate row. The generator measures East-Asian display width and rejects any non-divider content row above 20 display columns; long fields move to labeled continuation rows before validation. ISO/slash date ranges are omitted so LINE does not turn them into distracting blue links. Do not paste the verbose engine `reason` strings back into LINE or flatten the grouped layout into a continuous bullet list. The three decisions are: `買進觀察` for a fresh disclosed entry/re-entry/reversal/restart, `續抱參考` for exceptional continuing conviction, and `賣出警示` for a fresh exit/reversal/restart. A `new_position` in one daily comparison means newly present, not necessarily first-ever; if that ETF has any earlier `full_exit` in its full available history, label the event `重新建倉`. Cross-ETF directional comparisons remain restricted to common sessions. Fresh buy/sell events live for exactly the latest **2 common ETF sessions**: the UI labels them `本交易日確認` then `前一交易日確認`, and they disappear on the next common session; there must never be a `3 日前確認` state. The hold lane is different: `最新資料仍確認` is recalculated from scratch every day and is not an old event retained by age. Ordinary adjustments must clear `max(0.02% ActiveWeight, 35% of that ETF's trailing trade median)`; tiny full-exit residue is hidden. **Qualification is fixed and user-visible:** an ordinary fresh action requires at least 2/3 ETFs aligned. A 1/3 action is hidden unless it is a holding-list change (`新建倉／重新建倉／完整出清`) or a genuine reversal (`賣後轉買／買後轉賣`) that persists for two common sessions. A single-ETF ordinary restart must not surface. Ordinary continuations stay out of fresh buy/sell lanes; the hold lane requires participation by at least 2 ETFs, at least 4 meaningful buy days in the latest 10 common sessions, buy days at least 2 greater than sell days, normalized net flow of at least 0.20, and a latest-session buy confirmation. Both the web page and LINE header must state the 2/3 rule and the narrow 1/3 exceptions. The page deliberately has no date interval, chart, table, or cash-size ranking. 類股 is context only; 概念 is never interpreted. Never infer that a trade occurred at a price high/low from ETF holdings alone; price timing requires separately verified market prices.
