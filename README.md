...
| `ETF共識` / `ETF意圖` | Exactly two cached 1800px-wide image reply objects and no text: buy top five, then sell top five. Each image is a balanced two-column rank summary; yellow one-manager watches and every rank below five remain website-only. A 180px pure-black top safe area keeps iPhone's Dynamic Island from covering the title in full-screen viewing. The webhook only validates and serves the manifest cache; it never calculates or screenshots on demand. |
...
| `scripts/generate_etf_consensus_v4_summary.py` | Renders exactly two 1800px-wide LINE decision summaries: buy top five and sell top five. Each is split into balanced side-by-side rank ranges, keeps core/tracking card badges, and begins with a 180px black iPhone safe area. The complete yellow/red/green board is never truncated on the website; only LINE is intentionally filtered. |
...
| `data/summaries/etf_consensus_v4_{buy,sell}_top5_latest.jpg` + `etf_consensus_v4_manifest.json` | Two wide V4 LINE decision summaries in buy/sell order. The manifest is the sole ordered reply list; no text object is included. Images and manifest are generated/ignored artifacts, not tracked source. |
...
21. **`ETF 共識 V4` is the final copy-the-manager contract.** Colour and score have different jobs. Yellow/watch is allowed only for one-manager structural/re-entry/full-exit/reversal/restart evidence or a former consensus that lost its second manager. To avoid tiny holding-list cleanup washing the lane, new/re-entry needs at least `0.6×` that ETF's usual single action and full exit needs `1.0×`; reversal/restart already requires the complete significance gate. Red/green require at least **2/3 ETFs**, each already significant, with same-direction signal lifetimes overlapping within three common sessions. A score—however large—must never upgrade 1/3 to red/green.
    ...
    **Core is a priority layer, never a replacement for the hard state.** A red/green card enters `core` only when score ≥60, freshness ≥8, relative strength ≥8, 3/10/20 alignment ≥3, and it is either repeatedly confirmed (joint-persistence component ≥10) or a strong formation no older than three state sessions. Everything else remains visible as valid `tracking`. This protects the user from choice overload without pretending that deleting cards improves the underlying evidence.
    ...
    The LINE command/rich-menu reply contains no text, fetch, on-demand screenshot, or paid push. It reads the generated manifest and sends exactly **two 1800px-wide, two-column cached images**: buy top five then sell top five. Rank is the existing same-colour consensus-score order; core/tracking remains a card-level badge. Yellow and ranks below five are intentionally website-only. Both images start with a 180px pure-black safe area for iPhone Dynamic Island viewing. Never restore the full board to LINE.
