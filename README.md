...
| `src/tag_flow_rotation.py` | Shared, window-independent 類股 rotation engine used by both the dashboard and the LINE/email card. It combines 3/10/20-session EWMA pressure, each category's own historical magnitude, ETF breadth, and two-session state confirmation. The 3-session EWMA is also the early current-pressure warning; there is no separate literal 3-day sum. User surfaces translate raw percentiles into plain `自身力道強／中／一般`. It never reads 概念 labels and never fetches data. |
| `src/tag_flow_events.py` | Stock-level V2 buy/hold/sell engine. Fresh signals include new positions, re-entry after a prior full exit, meaningful full exits, confirmed reversals against prior 20-session behaviour, and restarts after at least 5 inactive sessions. A separate hold lane admits only unusually persistent buying: at least 4 meaningful buy days in the latest 10 common sessions, clearly more buy than sell days, sufficient normalized cumulative strength, and buying still confirmed on the latest session. Ordinary small adjustments remain excluded. Structural history uses every available date per ETF; cross-ETF direction comparisons use common dates. Category is context only and concepts are never interpreted. |
...
| `src/ui/tag_flow_tab.py` | 題材流向 tab: a concise three-lane board (`綜合買盤已確認／綜合轉強待確認／3日 EWMA 目前減碼警示`) plus a category trend drill-down. The heading must say `最新…截至 YYYY-MM-DD`, never `今日`, because lane assignment is a 3/10/20-half-life combined state rather than a one-day result. Every board card shows the same 3-session EWMA current pressure and ETF buy/sell breadth; there is no literal recent-3-day total. Each card also shows up to three small `留意個股` names ranked by contribution to that card's direction: positive 3-session-EWMA contributors for red/amber cards and negative contributors for green cards. The evidence flow is fixed: **② choose the audit interval → ③ show the interval category ranking openly (never hide it in an expander) → select a category from that ranked interval → show its trend and ④ interval-specific stock sources**. The selector changes only the ranking/chart/audit totals, never the category phase or LINE conclusion. The visible presets deliberately omit 3 and 5 sessions; only supported 1/10/20/60/120/240 ranges appear. Ranks normalized 相對力道 for fair cross-fund comparison and shows estimated 億元 only as intuitive context. Taiwan colors: red = 加碼/買進, green = 減碼/賣出, amber = pending. 概念 labels are stock-level notes only. Pure render of `data/tag_flow.json` (no network). |
| `src/ui/tag_flow_v2_tab.py` | Separate `題材流向 V2` tab; V1 remains intact. This is an insight-only three-lane stock radar (`買進觀察／續抱參考／賣出警示`) with no interval selector and no charts. It shows at most six fresh events per buy/sell side and four exceptional continuing-buy signals in the hold lane, their single 類股 as a context pill, the ETFs involved, and plain-language evidence. |

題材流向 periods mean the latest **common trading sessions** across the selected ETFs, not calendar days; 20 trading sessions naturally span about one calendar month. If 60/120/240 exceeds the current shared history, those buttons stay hidden rather than silently showing the same one-month data. They appear automatically after enough common sessions accumulate. The selector is immediately above the category chart and is a chart/audit viewport only. The rotation story always uses all available common history through `src/tag_flow_rotation.py`, so switching display ranges cannot rewrite the headline.

The rotation model deliberately has no decision interval. It gives every old session a gradually fading weight instead of dropping an entire buy wave at a boxcar boundary: **3-session EWMA = current pressure, 10-session EWMA = underlying direction, and 20-session EWMA = background**. These are decay speeds, not literal truncated day windows. "Strong/weak" magnitude is relative to that category's own prior smoothed-pressure history; a cross-category rank is secondary context. A coordinated buy/sell claim needs at least two selected ETFs pointing the same way, and a new phase must persist for two common sessions before it replaces the confirmed state. An abrupt reduction is surfaced when the 3-session EWMA current pressure turns materially negative—even while the 10/20-session EWMAs remain positive—without falsely calling that a confirmed long-term downtrend. The dashboard, daily email, generated image, and LINE reply must all call this shared engine. The selector's interval totals and the image's last-ten-session mini bars remain display/audit evidence only and never alter the model conclusion.
...
| `題材洞察` | Latest decision-only image card from the shared window-independent rotation engine: confirmed buys/sells, the strongest 3-session-EWMA current-pressure warning, and strict 3/3 common buy/sell stock pools. Raw P-values are never shown; strength uses `自身力道強／中／一般`. The ten-session mini bars are visual history only. Available as a visible Page 1 rich-menu tile and an 吳大師 quick reply. |
...
| `scripts/generate_tag_flow_insight.py` | Converts `data/tag_flow.json` through the shared window-independent rotation engine into `data/tag_flow_insight.json` plus `data/summaries/tag_flow_insight_latest.jpg` for the daily admin email, scheduled LINE image, and on-demand `題材洞察` reply. The card shows confirmed buy/sell phases plus the strongest green 3-session-EWMA current-pressure warning, with last-ten-session normalized mini bars labeled visual-only. Common stock buy/sell pools require all three ETFs to agree. |
...
| `data/tag_flow_insight.json` | Shared rotation-engine output (schema v2) used by email and LINE: confirmed category phases, confidence/breadth, own-history strength, the shared 3/10/20 EWMA state, display-only 10/20-session audit totals and last-ten-session bars, and the rendered text payload. Rebuilt daily by `scripts/generate_tag_flow_insight.py`. |
...
17. **Dashboard, email, and LINE 題材洞察 share one window-independent V1 rotation engine.** `src/tag_flow_rotation.py` is the only source of V1 category phase, ranking, breadth, confidence, 3-session-EWMA current-pressure warnings, and common-stock pools. `scripts/generate_tag_flow_insight.py` writes both `data/tag_flow_insight.json` and the tracked `data/summaries/tag_flow_insight_latest.jpg`; the daily email prints `email_text`, while the scheduled 18:30 LINE run and on-demand `題材洞察` reply serve the image. Never copy V1 decision logic into the UI or webhook. The engine uses all common history with EWMA half-lives 3/10/20 for current pressure/underlying direction/background, judges magnitude against that category's own prior pressure history, requires majority-ETF direction breadth, and confirms a changed phase only after two consecutive common sessions. User-facing cards use `自身力道強／中／一般`, never unexplained P80/P64 labels. The graph selector belongs directly above the trend chart; unsupported 60/120/240 ranges remain hidden until enough common history exists. Changing the graph range must not change phase, rank, or LINE wording. A stock enters `三檔共買池` or `三檔共賣池` only when 00403A, 00981A, and 00991A all agree under the same smoothed current-pressure rule. Concepts remain stock-level display notes only. Never run the full daily script merely to test this card because that would send the paid broadcast; render the generator directly and inspect the JPG instead.

18. **題材流向 V2 is a buy/hold/sell evidence radar, not another level/ranking dashboard.** Keep V1 and V2 separate. `src/tag_flow_events.py` is the only source of V2 evidence, and `src/ui/tag_flow_v2_tab.py` only renders it. The three decisions are: `買進觀察` for a fresh disclosed entry/re-entry/reversal/restart, `續抱參考` for exceptional continuing conviction, and `賣出警示` for a fresh exit/reversal/restart. A `new_position` in one daily comparison means newly present, not necessarily first-ever; if that ETF has any earlier `full_exit` in its full available history, label the event `重新建倉` and state the prior exit date. Cross-ETF directional comparisons remain restricted to common sessions. Ordinary adjustments must clear `max(0.02% ActiveWeight, 35% of that ETF's trailing trade median)`; tiny full-exit residue is hidden. Non-structural direction changes require either 2 ETFs on the same disclosure or the same direction for 2 consecutive sessions. Ordinary continuations stay out of fresh buy/sell lanes; the hold lane requires at least 4 meaningful buy days in the latest 10 common sessions, buy days at least 2 greater than sell days, normalized net flow of at least 0.20, and a latest-session buy confirmation. Show at most six fresh events per side and four hold signals. The page deliberately has no date interval, chart, table, or cash-size ranking. 類股 is a context label only; 概念 is never interpreted. Never infer that a trade occurred at a price high/low from ETF holdings alone; price timing requires separately verified market prices.

19. **融資風險 is an estimate, not an official account-average series.** The exchanges publish margin-share balances, prices, and aggregate financing amounts, but not every brokerage customer's supplementary collateral. Its numerator must exclude the exchange `00...` ETF code family while its denominator retains the official aggregate financing balance, matching MacroMicro's published ETF-exclusion principle; do not silently put ETFs back into the numerator. Always label the result `全市場融資擔保估算率` / `公開資料估算`; never call it the official `台股平均融資維持率`, never claim it reproduces MacroMicro, and never describe 130% as this aggregate line's exact forced-liquidation trigger. Only `scripts/update_margin_maintenance.py` may call TWSE/TPEx. The dashboard and webhook are cache-only. Red means improving buffer and green means worsening buffer, per the site's Taiwan color convention. The `吳大師` card must keep both quick replies (`今日類股洞察` and `融資風險`).
