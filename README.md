...
| `src/ui/tag_flow_v2_tab.py` | `ETF 動作` tab (the filename is historical): an insight-only three-lane stock radar (`買進觀察／續抱參考／賣出警示`) with no interval selector and no charts. It shows the engine's complete qualified board (up to six fresh events per buy/sell side and four exceptional continuing-buy signals in the hold lane), their single 類股 as context, the ETFs involved, and plain-language evidence. Its lane/card renderer is shared with the three cached on-demand LINE images. |
...
| `scripts/generate_etf_action_insight.py` | Converts `data/tag_flow.json` through the stock-level action engine into cached `data/etf_action_insight.json` (schema v2). It preserves the engine's complete qualified board; `line_text` remains the concise copy embedded in the scheduled daily report's first text object and admin email. Every stock uses the fixed `類股／動作／ETF／判定／依據` field order. It never sends LINE itself. |
...
8. Run a live Cmoney category canary, validate category coverage, build the shared flow observations, generate the complete stock-level `ETF 動作` cache, then render its three mobile LINE lane images.
...
The rich menu uses three LINE rich-menu aliases. Page 1 is fast market/macro plus direct `市場脈動`, `吳大師`, and `ETF 動作` actions; Page 2 is the primary ETF watchlist, and Page 3 is ETF overflow. Navigation convention is fixed: previous page is bottom-left, next page is bottom-right, and the last page uses bottom-right `首頁` until another page is needed. Page 1's narrow five-column row deliberately uses short labels (`買・抱・賣`, `那斯達克` + `24 小時`); both title and subtitle fonts are width-fitted per tile. Do not restore long one-line labels that overflow on a phone.
...
18. **`ETF 動作` is the actionable buy/hold/sell evidence radar and the only ETF-flow insight sent to LINE.** `src/tag_flow_events.py` is the single evidence source; the website and `scripts/generate_etf_action_summary.py` share `src/tag_flow_action_cards.py`. The on-demand `ETF動作` reply is currently exactly **3 mobile-width vertical images**, ordered buy, hold, sell. The cached text and `load_etf_action_text()` remain intact for later reuse, but `ETF_ACTION_INCLUDE_TEXT = False` keeps it out of the reply; changing that single constant to `True` restores text-first delivery. Each image renders one complete website lane only—lane title/count/note plus all cards, with no repeated page title, date summary, methodology boxes, or footer. The renderer must verify each lane's `.tfv2-card` count against `data/etf_action_insight.json`, capture that lane body's computed height, and never use a fixed-height crop, `max-height`, or `overflow:hidden`; every lane's last card must be present. The webhook only serves `data/summaries/etf_action_buy_latest.jpg`, `etf_action_hold_latest.jpg`, and `etf_action_sell_latest.jpg` and must never render or recalculate during a reply. The scheduled daily broadcast remains different and unchanged: one combined report/action text plus exactly four active-ETF images. Text and admin email retain the fixed field order `名稱｜代號`, `類股`, `動作`, `ETF`, `判定`, `依據`; do not restore event-specific labels or automatic wrapping. The three decisions are: `買進觀察` for a fresh disclosed entry/re-entry/reversal/restart, `續抱參考` for exceptional continuing conviction, and `賣出警示` for a fresh exit/reversal/restart. A `new_position` in one daily comparison means newly present, not necessarily first-ever; if that ETF has any earlier `full_exit` in its full available history, label the event `重新建倉`. Cross-ETF directional comparisons remain restricted to common sessions. Fresh buy/sell events live for exactly the latest **2 common ETF sessions**: the UI labels them `本交易日確認` then `前一交易日確認`, and they disappear on the next common session; there must never be a `3 日前確認` state. These time labels are neutral secondary pills: the current-session pill has a subtle filled treatment, while the prior-session pill is dimmer and outlined; neither may reuse or compete with the red/orange/green lane colors. The hold lane is different: `最新資料仍確認` is recalculated from scratch every day and is not an old event retained by age. Ordinary adjustments must clear `max(0.02% ActiveWeight, 35% of that ETF's trailing trade median)`; tiny full-exit residue is hidden. **Qualification is fixed and user-visible:** an ordinary fresh action requires at least 2/3 ETFs aligned. A 1/3 action is hidden unless it is a holding-list change (`新建倉／重新建倉／完整出清`) or a genuine reversal (`賣後轉買／買後轉賣`) that persists for two common sessions. A single-ETF ordinary restart must not surface. Ordinary continuations stay out of fresh buy/sell lanes; the hold lane requires participation by at least 2 ETFs, at least 4 meaningful buy days in the latest 10 common sessions, buy days at least 2 greater than sell days, normalized net flow of at least 0.20, and a latest-session buy confirmation. Every stock card carries one fixed 20-common-session micro bar chart beneath its evidence: each day sums the three ETFs' fund-size-normalized `flow` (never absolute cash), red is net buy, green is net sell, and neutral evidence markers land only on the session(s) that actually qualify the signal: one frame for a single-session confirmation, a connected frame for a two-session reversal, a dot for a holding-list change, a dashed frame for an idle restart, and a latest-session frame plus a faint 10-session bracket for continuing conviction. The vertical scale is local to that stock, so the chart communicates sequence and reversal shape, not magnitude ranking across stocks; it must have no interval selector or extra analytical panel. The optional LINE text states the 2/3 rule and narrow 1/3 exceptions; the three images avoid repeating that explanation. Apart from this evidence sparkline, the page deliberately has no interval-driven chart, table, or cash-size ranking. 類股 is context only; 概念 is never interpreted. Never infer that a trade occurred at a price high/low from ETF holdings alone; price timing requires separately verified market prices.

19. **融資風險 is an estimate, not an official account-average series.** The exchanges publish margin-share balances, prices, and aggregate financing amounts, but not every brokerage customer's supplementary collateral. Its numerator must exclude the exchange `00...` ETF code family while its denominator retains the official aggregate financing balance, matching MacroMicro's published ETF-exclusion principle; do not silently put ETFs back into the numerator. Always label the result `全市場融資擔保估算率` / `公開資料估算`; never call it the official `台股平均融資維持率`, never claim it reproduces MacroMicro, and never describe 130% as this aggregate line's exact forced-liquidation trigger. Only `scripts/update_margin_maintenance.py` may call TWSE/TPEx. The dashboard and webhook are cache-only. Red means improving buffer and green means worsening buffer, per the site's Taiwan color convention. The `吳大師` card must keep both quick replies (`ETF 動作` and `融資風險`).
