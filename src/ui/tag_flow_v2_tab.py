...
def render_tag_flow_v2_tab(*, DATA_DIR=None, **kwargs):
    ...
    st.caption(
        "買＝新觸發或觸發後今日續買；抱＝接近門檻、有效續抱或加碼動能降溫；"
...
<div class="tfv2-rule"><b>續抱是滾動狀態，不是每日買賣指令</b><span>先以各 ETF／個股前 10 日慣常動作判斷每一筆是否顯著，再用最近 10 個共同交易日確認續抱強度；降溫但沒有顯著賣出時，仍留在續抱區。</span></div>
</div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.caption(
        ...
        "會明寫『昨日觸發・今日續買／續賣』。續抱每天用滾動證據重算，"
        "並保留剛轉為降溫的一日提示。每張卡使用同一組狀態欄位，普通小額調整仍隱藏。"
...
- **證據標記**：實線框是單日確認；連接框是兩日反轉；框上圓點是建倉／出清等持股名單改變；虛線框是沉寂後重啟。續抱圖保留完整 20 日背景，只在真正納入目前判斷的顯著加／減碼日放證據點，並淡框最近一次證據；最新交易日沒有動作就完全不框。
- **續抱參考**：每一筆動作先和該 ETF／該股／同方向的前 10 個共同交易日比較，通過門檻才算證據；續抱狀態再用最近 10 個共同交易日重算。至少 2 檔 ETF 曾參與、至少 4 日顯著加碼、買日比賣日至少多 2 日、規模比淨買至少 +0.20%。20 日圖是前後文，不代表只看框內；卡片會顯示安靜幾日後強度降溫。
- **升級／降溫**：只差一次且其他續抱條件已達標時顯示升級觀察；剛跌出強勢續抱門檻時顯示「續抱降溫」，不等於退出或賣出。只有同時形成合格賣出訊號，才會移到「賣出警示」。
